    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if(player.getHeldItemMainhand() != null && !world.isRemote) {
    		if(player.getHeldItemMainhand().getItem() instanceof TubbyBowl) {
    			...
    			int size = player.inventory.getStackInSlot(slot).getCount();
    			ItemStack stack0 = player.inventory.getStackInSlot(slot);
    			ItemStack custard = new ItemStack(Teletubbies.tubbyCustard, 1);
    			TileEntityTubbyCustardMachine tem = (TileEntityTubbyCustardMachine) world.getTileEntity(new BlockPos(te.getMasterX(), te.getMasterY(), te.getMasterZ()));
    			    			
    			if(tem.canDrop()) {
    				stack0.setCount(stack0.getCount() - 1);
    	    		world.playSound(null, pos, Teletubbies.sound_TubbyCustard, SoundCategory.NEUTRAL, 1, 1);
		...
		if(player.getHeldItemMainhand() != null && !(player.getHeldItemMainhand().getItem() instanceof TubbyBowl) && world.isRemote){
