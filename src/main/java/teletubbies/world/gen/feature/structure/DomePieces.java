...
public class DomePieces {
	...
	public static class Piece extends TemplateStructurePiece {
	    ...
	    public Piece(TemplateManager manager, ResourceLocation rl, BlockPos pos) {
	        super(WorldGenList.DOME_PIECE, 0);
	    ...
	    @Override
	    public boolean create(IWorld world, ChunkGenerator<?> generator, Random rand, MutableBoundingBox mbb, ChunkPos chunkPos) {
	    	boolean b = super.create(world, generator, rand, mbb, chunkPos);
	        	...
	        	for (int i = 0; i < mbb.getXSize(); i++) {
	        		for (int j = 0; j < mbb.getZSize(); j++) {	        			
	        			if (this.getBlockStateFromPos(world, i, 0, j, mbb).getBlock() instanceof FullGrassBlock ||
	        				...
	        				this.replaceAirAndLiquidDownwards(world, BlockList.FULL_GRASS.getDefaultState(), i, -1, j, mbb);
		...
		@Override
		protected void handleDataMarker(String function, BlockPos pos, IWorld world, Random rand, MutableBoundingBox mbb) {
		...
		protected void replaceAirAndLiquidDownwardsThrough(IWorld world, BlockState blockstateIn, int x, int yHigh, int z, int yLow, MutableBoundingBox mbb) {
			...
			if (mbb.isVecInside(new BlockPos(i, j, k))) {
				while (j >= low) {
					...
					else if ((world.getBlockState(new BlockPos(i, j, k)).has(BlockStateProperties.WATERLOGGED)) && j > 1) {
