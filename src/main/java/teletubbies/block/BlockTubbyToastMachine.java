...
public class BlockTubbyToastMachine extends Block implements ITileEntityProvider {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if(!world.isRemote) {
            ...
            if(tem.canDrop()) {
            	dropToast(new ItemStack(Teletubbies.tubbyToast, 1), world, te.getMasterX(), te.getMasterY(), te.getMasterZ(), player);
	...
	public void dropToast(ItemStack itemstack, World world, int x, int y, int z, EntityPlayer player) {
    	if(!world.isRemote) {
