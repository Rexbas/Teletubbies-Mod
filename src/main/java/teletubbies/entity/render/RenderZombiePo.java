...
public class RenderZombiePo extends RenderBiped<EntityZombiePo> {
    ...
    private final List<LayerRenderer<EntityZombiePo>> defaultLayers;
	...
	private void swapArmor(EntityZombiePo entity) {
		this.mainModel = this.defaultModel;
		this.layerRenderers = this.defaultLayers;
		this.modelBipedMain = (ModelBiped) this.mainModel;
	}
	
    protected void rotateCorpse(EntityZombiePo entityLiving, float f0, float f1, float partialTicks) {
        if(entityLiving.isConverting()) {
            f1 += (float)(Math.cos((double)entityLiving.ticksExisted * 3.25D) * Math.PI * 0.25D);
        }
        super.rotateCorpse((EntityZombiePo) entityLiving, f0, f1, partialTicks);
    }
