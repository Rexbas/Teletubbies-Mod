...
public class RenderZombieTinkyWinky extends RenderBiped<EntityZombieTinkyWinky> {

	private final ModelBiped defaultModel;
    private final List<LayerRenderer<EntityZombieTinkyWinky>> defaultLayers;
	
	public RenderZombieTinkyWinky(RenderManager rendermanager, boolean isZombie, final float scale, float shadowsize) {
		super(rendermanager, new ModelTinkyWinky(isZombie, scale), shadowsize);
	...
	@Override
    public void doRender(EntityZombieTinkyWinky entity, double x, double y, double z, float entityYaw, float partialTicks) {
    	this.swapArmor(entity);
    	super.doRender(entity, x, y, z, entityYaw, partialTicks);
	}
	
	private void swapArmor(EntityZombieTinkyWinky entity) {
		this.mainModel = this.defaultModel;
		this.layerRenderers = this.defaultLayers;
		this.modelBipedMain = (ModelBiped) this.mainModel;
	}
	
    protected void rotateCorpse(EntityZombieTinkyWinky entityLiving, float f0, float f1, float partialTicks) {
        if(entityLiving.isConverting()) {
            f1 += (float)(Math.cos((double)entityLiving.ticksExisted * 3.25D) * Math.PI * 0.25D);
        }
        super.rotateCorpse((EntityZombieTinkyWinky) entityLiving, f0, f1, partialTicks);
    }

	@Override
	protected ResourceLocation getEntityTexture(EntityZombieTinkyWinky entity) {
		return new ResourceLocation("teletubbies:textures/entity/ZombieTinkyWinky.png");
	}
}